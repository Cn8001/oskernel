...
struct fat_file_descriptor{     //Fopen için gereken şey (pos -> dosyanın neresindeyiz)
...
int fat16_get_total_items_for_directory(struct disk* disk,uint32_t directory_start_sector){
    ...
    if(diskstreamer_seek(stream,directory_start_pos) != HESOS_ALL_OK){
        res = -EIO;
        goto out;
    }
    while(1){
        if(diskstreamer_read(stream,&item,sizeof(item)) != HESOS_ALL_OK){
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk,struct fat_private* private, struct fat_directory* directory){
    ...
    //Eğer tam bölünmüyorsa bir sektör arttırıyoruz çünkü daha veri var.
    ...
    if(diskstreamer_seek(stream,fat16_sector_to_absolute(disk,root_dir_sector_pos)) != HESOS_ALL_OK){
        res = -EIO;
        goto out;
    }

    if(diskstreamer_read(stream,dir,root_dir_size) != HESOS_ALL_OK){
        res = -EIO;
        goto out;
    }
    directory->item = dir;
...
int fat16_resolve(struct disk* disk){
    ...
    if(diskstreamer_read(stream,&private->header,sizeof(private->header)) != HESOS_ALL_OK){
        res = -EIO;
        goto out;
    }
    ...
    if(fat16_get_root_directory(disk,private,&private->root_directory) != HESOS_ALL_OK){
        res = -EIO;
        goto out;
    }
...
void fat16_proper_to_string(char** out, const char* in){
...
void fat16_get_full_relative_filename(struct fat_directory_item* item,char* out, int max_len){ //test txt -> test.txt
    memset(out,0x00,max_len);
    char *out_tmp = out;
    fat16_proper_to_string(&out_tmp,(const char*)item->filename);
    if(item->ext[0] != 0x00 && item->ext[0] != 0x20){
        *out_tmp++ = '.';
        fat16_proper_to_string(&out_tmp,(const char*)item->ext);
    }
}
...
static uint32_t fat16_get_first_fat_sector(struct fat_private* private){
    //Ilk cluster reserved sectorsten sonra geliyor.
...
static int fat16_get_fat_entry(struct disk* disk, int cluster){
    ...
    res = diskstreamer_seek(stream,fat_table_position * (cluster * HESOS_FAT16_ENTRY_SIZE));
    if(res < 0){
        goto out;
    }
    uint16_t result = 0;
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset){
    ...
    for(int i = 0;i<clusters_ahead;i++){
        ...
        if(entry == HESOS_FAT16_BAD_SECTOR){
            res = -EIO;
            goto out;
        }
        ...
        cluster_to_use = entry; //Her clusterda sondaki cluster adresi var
    }
    res = cluster_to_use;
out:
    return res;
...
static int fat16_read_internal_from_stream(struct disk* disk,struct disk_stream* stream, int cluster, int offset, int total,void* out){
    ...
    int offset_from_cluster = offset % size_of_cluster_bytes; //4097 % 4096 = 1 (1. cluster)
    int starting_sector = fat16_cluster_to_sector(private,cluster_to_use);
    int starting_pos = starting_sector * disk->sector_size * offset_from_cluster;

    //Her seferinde bir cluster okuyabiliriz.
...
static int fat16_read_interval(struct disk* disk, int starting_cluster, int offset, int total, void* out){
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item){
    ...
    struct fat_private* fat_private = disk->fs_private;
    //Dir yükleyebiliriz file gönderirse bozar.
    ...
    res = fat16_read_interval(disk,cluster,0x00,directory_size,directory->item);
    if(res < 0){
        res = -ENOMEM;
        goto out;
    }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk,struct fat_directory* directory,const char* name){
    ...
    char tmp_filename[HESOS_MAX_PATH]; 
    for(int i=0; i<directory->total;i++){
        fat16_get_full_relative_filename(&directory->item[i],tmp_filename,sizeof(tmp_filename));
        if(istrncmp(tmp_filename,name,sizeof(tmp_filename)) == 0){
            //Gelen input doğru fat_item oluşturalım
